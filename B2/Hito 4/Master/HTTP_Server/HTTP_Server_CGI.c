...
//Hilos:
extern osThreadId tid_ThreadFlash; 
extern osThreadId tid_ThreadMasterI2C; 
...
extern char ov[20+1];
extern uint8_t interrupcionOverload;
    ...
    if (var[0] != 0) {
      ...
      if (strcmp (var, "led0=on") == 0) {
        ...
        //LEDrun = false;
			...
			/***********************************************************************************************************************************
			 * EXPLICACION: el servidor carga toda la pagina en la que se encuentra cada vez que se realiza una pulsacion o modificacion       *
			 * de alguno de los botones o cajas de texto. Por lo que añadimos los casos correspondientes para el nuevo caso AGP con diversos   *
			 * if-else if.																																																										 *
			 ***********************************************************************************************************************************/
			
      else if (strcmp (var, "ctrl=1") == 0) //GANANCIA.
      {
				ganancia = 1;
				tx_over_gain = 1;
      ...
      else if (strncmp (var, "over1=", 5) == 0) //OVERLOAD
			{
      ...
      else if (strcmp (var, "ctr=Si") == 0) //INTERRUPCION OVERLOAD.
      {
        intOverload = 1;
				
				tx_over_gain = 3;
      }
      else if (strcmp (var, "ctr=No") == 0)
      {
        intOverload = 0;
  ...
  switch (env[0]) {
    ...
    case 'g':
      ...
      break;
			...
			case 'h': //Se añade un nuevo caso para mostrar la hora y la fecha con SNTP.
		
			switch (env[2])
			{				
				case '1':	//OJO CON LAS COMILLAS SIMPLES, SI NO SE PONEN NO ENTRA EN CADA CASE Y NO SE VE NADA EN EL SERVER!!!!!!!!!!!!!!
      ...
      break;
          ...
          len = sprintf (buf, &env[4],envio);
				
          break;

				case 'd': //Overload.
			    
				  len = sprintf (buf, &env[4], ov);
				 
				  break;						

				case 'e': //Overload S/N.
			    
				  len = sprintf (buf, &env[4],interrupcionOverload);
