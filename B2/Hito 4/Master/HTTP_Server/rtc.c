...
void RTC_getTime_Date(void)
{
  ...
  ano = RTC_GetTime(LPC_RTC, RTC_TIMETYPE_YEAR);
	...
	sprintf(cadenaReloj,"%.2d:%.2d:%.2d", horas, minutos, segundos); //Estas dos cadenas son para la pagina del servidor.		
	sprintf(cadenaFecha,"%.2d/%.2d/%.2d", dia, mes, ano);
	
	//Una vez hemos obtenido el tiempo, limpiamos el LCD y con "sprintf"... 
 //...escribimos la hora.
	
	limpiardisplay();
  EscribeGanancia(ganancia);
...
//	RTC_SetTime (LPC_RTC, RTC_TIMETYPE_DAYOFMONTH, 1);
   ...
   * en el archivo lpc17xx_rtc.c, pasamos como parametro la unidad de tiempo para la que queremos que salte la alarma      *
	...
	RTC_SetAlarmTime (LPC_RTC, RTC_TIMETYPE_SECOND, 0);// Alarma de 1 minuto.
...
//Funcion que actualiza el tiempo con valores directos:
void RTC_JOY(void)
{
 	RTC_SetTime (LPC_RTC, RTC_TIMETYPE_SECOND, 0);
	RTC_SetTime (LPC_RTC, RTC_TIMETYPE_MINUTE, 0);
