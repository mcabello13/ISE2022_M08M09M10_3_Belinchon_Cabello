...
/******************************************************************************************
* ESTE PROYECTO IMPLEMENTA SNTP, CGI Y GUARDADO EN FLASH PERFECTO FUNCIONAMIENTO.         *
*******************************************************************************************/
...
char overload [2];
char ganancia_combo1;
char ganancia_combo2;
char g_o[2][20+1];
int ganancia;
char lcd_buf[20+1]; 
int i;
IAP_STATUS_CODE status;
uint8_t bufFlash [12] = {ETH0_IP1, ETH0_IP2, ETH0_IP3, ETH0_IP4, ETH0_MAC1, ETH0_MAC2, ETH0_MAC3, ETH0_MAC4, ETH0_MAC5, ETH0_MAC6};
...
//Funcion que inicializa la Flash (Escribe IP y MAC y comprueba el estado de los LED):
void flash (void)
{	
  uint32_t flash_prog_area_sec_start;
  uint32_t flash_prog_area_sec_end;
  ...
  flash_prog_area_sec_end =  GetSecNum(FLASH_PROG_AREA_START + FLASH_PROG_AREA_SIZE); 
	...
	{		
		if((modoLeds <= 0x0F)) //...se comprueba que LED estan ON y OFF.
		{
			if(modoLeds == 0x01)
			{
	...
	else //...si no es Manual, es modo Barrido.
	{
		LEDrun = true;
	}
}
