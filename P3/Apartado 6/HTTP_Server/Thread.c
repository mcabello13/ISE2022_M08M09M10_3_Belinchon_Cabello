...
void ThreadFlash (void const *argument) 
{
	...
	{		
		...
		bufFlash[10] = modoLeds; //En la posicion 10, se guarda el estado de los LED, ya que la otras 9 posiciones estan ocupadas por IP + MAC.
		...
		for (k=0; k<FLASH_PROG_AREA_SIZE/BUFF_SIZE; k++) //Se realiza la copia cuatro veces --> 1000/256.
		{
			ptr = (uint8_t*)(FLASH_PROG_AREA_START + k*BUFF_SIZE);
